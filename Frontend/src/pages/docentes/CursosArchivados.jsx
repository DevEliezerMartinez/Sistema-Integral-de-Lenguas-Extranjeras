...
function CursoArchivado() {
  ...
  useEffect(() => {
    const fetchCursosArchivados = async () => {
      ...
      if (docente && token) {
        try {
          const response = await axios.get(`http://127.0.0.1:8000/api/cursosArchivados/${docente.id}`, {
            headers: {
          ...
          if (response.status === 200) {
            const dataCursos = response.data.cursos ? Object.values(response.data.cursos) : [];
      ...
      <div id="Contenedor de CARDS" className="flex gap-3 justify-center mt-5 flex-wrap">
        {loading ? (
          ...
          <div id="Card" className="border rounded bg-slate-100 w-3/5 flex flex-col px-8 py-4 items-center text-center">
            ...
            <p className="Montserrat font-normal">Sin cursos archivados disponibles</p>
            <span>
          ...
          cursosArchivados.map((curso) => (
            ...
            >
              ...
              <ul className="self-start text-left">
                ...
                <li>Periodo: {curso.fecha_inicio} - {curso.fecha_fin}</li>
              </ul>
